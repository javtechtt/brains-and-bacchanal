...
update without reload, refresh-reconnect keeping the same `playerId` and team,
and leaving invalidating the credential. Also fails on any JavaScript error.
...
Socket-lifecycle scenarios that unit tests cannot reach (Phase 4 spec §31):
...
- a displaced socket's `close` arriving **late**, which must not mark a present
  player as away
- Host removal, the 3→2 Team C guard, and lock validation
